...
function get_caption(tag) {
	return tag.textContent.toLowerCase().replace(/<\/?[^>]+(>|$)/g, "").replace(/\s+/g,' ').replace('&#39', "'").replace(';','').trim();
}
...
function init(captionsOn = "false") {
	try {
		let button = document.getElementsByClassName('ytp-subtitles-button ytp-button')[0];
		if (button.getAttribute("aria-pressed") == captionsOn) {
			button.click();
		}
	} catch(err) {
		console.log("FAILED");
...
chrome.runtime.onMessage.addListener(function(response, sender, send_response) {
	...
	if (response.captions != undefined) {
		...
		send_response({status: "Captions received", youtube_title: youtube_title});	
	}
